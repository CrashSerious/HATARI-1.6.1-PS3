...
//hack ps3 
//remap but0 et but1 to cross & square
...
static bool Joy_ReadJoystick(int nSdlJoyID, JOYREADING *pJoyReading)
{
	...
	pJoyReading->Buttons = SDL_JoystickGetButton(sdlJoystick[nSdlJoyID], 9/*0*/);
	/* Sets bit #1 if button #2 is pressed: */
	if (SDL_JoystickGetButton(sdlJoystick[nSdlJoyID], 8/*1*/))
