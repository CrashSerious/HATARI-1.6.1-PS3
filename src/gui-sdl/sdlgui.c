...
//hack ps3
			 ...
			 case SDL_MOUSEMOTION:				
						...
						if( rct.x>=dlgrect.x && ( rct.x< ( dlgrect.x +dlgrect.w) ) && rct.y>=dlgrect.y && ( rct.y< ( dlgrect.y +dlgrect.h) )  )
						{
							SDLGui_DrawDialog(dlg); //slow gui lag 		
							SDL_FillRect(pSdlGuiScrn, &rct, SDL_MapRGB(pSdlGuiScrn->format,0,0,0));
                                                	SDL_UpdateRect(pSdlGuiScrn, 	rct.x,	rct.y,10,10);
						}
